...
def is_verse(text: str) -> bool:
    if re.search(r'[1-3]*\s[A-Za-z]*\s[0-9]*:\s[1-9]*\s-\s[0-9]*', text) is not None:
        return True
    elif (re.search(r'[A-Za-z]*\s[0-9]*:\s[0-9]*\s-\s[0-9]*', text)) is not None:
        return True
    elif re.search(r'[1-3]*\s[A-Za-z]*\s[0-9]*:\s[0-9]*', text) is not None:
        return True
    elif (re.search(r'[A-Za-z]*\s[0-9]*:\s[0-9]*', text)) is not None:
        return True
    else:
        return False
...
def fill_verse(text: str) -> str:
    if re.search(r'[1-3]*\s[A-Za-z]*\s[0-9]*:\s[1-9]*\s-\s[0-9]*', text) is not None:
        return add_range_verse_text(re.search(r'[1-3]*\s[A-Za-z]*\s[0-9]*:\s[0-9]*\s-\s[0-9]*', text).group())
    elif (re.search(r'[A-Za-z]*\s[0-9]*:\s[0-9]*\s-\s[0-9]*', text)) is not None:
        return add_range_verse_text(re.search(r'[A-Za-z]*\s[0-9]*:\s[0-9]*\s-\s[0-9]*', text).group())
    elif re.search(r'[1-3]*\s[A-Za-z]*\s[0-9]*:\s[0-9]*', text) is not None:
        return add_verse_text(re.search(r'[1-3]*\s[A-Za-z]*\s[0-9]*:\s[0-9]*', text).group())
    elif (re.search(r'[A-Za-z]*\s[0-9]*:\s[0-9]*', text)) is not None:
        return add_verse_text(re.search(r'[A-Za-z]*\s[0-9]*:\s[0-9]*', text).group())
    else:
        pass

def add_verse_text(verse: str) -> str:
    # split the verse into book chapter and verse
    if(re.search(r"[0-9]*\s[a-zA-z]*",verse).group().strip() != ''):
        book = re.search(r"[0-9]*\s[a-zA-z]*",verse).group().strip()
    else:
        book = re.search(r"[a-zA-z]*",verse).group().strip()
    chapter = int(re.split(r"[a-zA-z]",verse.split(":",maxsplit = 2)[0])[-1].strip())
